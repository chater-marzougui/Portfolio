...
async function dev() {
  try {
    ...
    if (await fs.pathExists(config.htmlPath)) {
      let modified = await updateHtmlForDev(filteredCssFiles, sortedJsFiles);
      if (modified) {
...
async function updateHtmlForDev(cssFiles, jsFiles) {
  ...
  let modified = false;
  let cssUncommented = false;
  let bundleCommented = false;
  ...
  if (cssUncommented) {
    console.log("✅ Uncommented CSS files");
  }
  ...
  for (const cssFile of cssFiles) {
    ...
    if (commentedCssRegex.test(htmlContent)) {
      ...
      const activeCssRegex = new RegExp(
  ...
  // Extract current JS file order from HTML
  ...
  // Get desired order
  const desiredJsOrder = jsFiles.map((file) => {
    const relativePath = path.relative(".", file).replace(/\\/g, "/");
    return relativePath.replace("./", "");
  });

  // Only modify JS if order is different or files are missing
  const needsJsReorder =
    JSON.stringify(currentJsOrder) !== JSON.stringify(desiredJsOrder);

  if (needsJsReorder) {
    // Remove all existing individual JS script tags
    for (const cleanPath of desiredJsOrder) {
  ...
  // Test if the new html file is same as old html file
  if (modified) {
